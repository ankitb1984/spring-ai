 ...
 * @author Geng Rong
 */
public class MoonshotAiPropertiesTests {
	...
	public void chatProperties() {
		...
		// @formatter:off
			...
			.run(context -> {
				var chatProperties = context.getBean(MoonshotAiChatProperties.class);
				var connectionProperties = context.getBean(MoonshotAiCommonProperties.class);
	...
	void chatActivation() {
		new ApplicationContextRunner()
			...
			.withConfiguration(AutoConfigurations.of(SpringAiRetryAutoConfiguration.class,
					RestClientAutoConfiguration.class, MoonshotAiAutoConfiguration.class))
			.run(context -> {
				assertThat(context.getBeansOfType(MoonshotAiChatProperties.class)).isNotEmpty();
				assertThat(context.getBeansOfType(MoonshotAiChatClient.class)).isEmpty();
			});
		...
		new ApplicationContextRunner()
			...
			.run(context -> {
				assertThat(context.getBeansOfType(MoonshotAiChatProperties.class)).isNotEmpty();
				assertThat(context.getBeansOfType(MoonshotAiChatClient.class)).isNotEmpty();
			});
