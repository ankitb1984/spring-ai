...
public class MoonshotAiRetryTests {
	...
	private @Mock MoonshotAiApi moonshotAiApi;

	private MoonshotAiChatClient chatClient;
	...
	public void beforeEach() {
		...
		chatClient = new MoonshotAiChatClient(moonshotAiApi,
				MoonshotAiChatOptions.builder()
					.withTemperature(0.7f)
					.withTopP(1f)
					.withModel(MoonshotAiApi.ChatModel.MOONSHOT_V1_32K.getValue())
					.build(),
				retryTemplate);
	}

	@Test
	public void moonshotAiChatTransientError() {
		...
		ChatCompletion expectedChatCompletion = new ChatCompletion("id", "chat.completion", 789l, "model",
				List.of(choice), new MoonshotAiApi.Usage(10, 10, 10));

		when(moonshotAiApi.chatCompletionEntity(isA(ChatCompletionRequest.class)))
	...
	@Test
	public void moonshotAiChatNonTransientError() {
		when(moonshotAiApi.chatCompletionEntity(isA(ChatCompletionRequest.class)))
	...
	@Test
	public void moonshotAiChatStreamTransientError() {
		...
		when(moonshotAiApi.chatCompletionStream(isA(ChatCompletionRequest.class)))
			.thenThrow(new TransientAiException("Transient Error 1"))
			.thenThrow(new TransientAiException("Transient Error 2"))
	...
	@Test
	public void moonshotAiChatStreamNonTransientError() {
		when(moonshotAiApi.chatCompletionStream(isA(ChatCompletionRequest.class)))
				.thenThrow(new RuntimeException("Non Transient Error"));
